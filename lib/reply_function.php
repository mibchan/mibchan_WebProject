...
function show_freeboard_comment() {
    require('lib/connectdb.php');
    $sql = mysqli_query($conn, "select * from reply where kind='free' and num={$_GET['num']} order by created");
...
function show_infoboard_comment() {
    require('lib/connectdb.php');
    $sql = mysqli_query($conn, "select * from reply where kind='info' and num={$_GET['num']} order by created");
    while($reply = $sql->fetch_array())
    {
       $num = $reply['num'];
       $nickname = $reply['nickname'];
...
function delete_freeboard_comment() {
  session_start();
  require("lib/connectdb.php");
  $sql1 = "select * from reply where reply_num={$_GET['reply_num']}";
...
<?php
        ...
        <div id="file">
              <a href="file_open.php?num=<?=$num?>"><?=$file?></a>
        </div>
        <p>
  ...
  <body>
    <div id="board_read">
      <?php
        ...
        <div id="file">
              <a href="file_open.php?board=<?=$_GET['board']?>&num=<?=$num?>"><?=$file?></a>
        </div>
        <p>
        <form action="reco.php?">
          <input type="hidden" name="board" value=<?=$_GET['board']?>>
...
function modify_comment() {
  ...
  $board = $reply['kind'];
  $num = $reply['num'];
  $sql2 = "update reply set comment='{$comment}' where reply_num='{$reply_num}'";
  mysqli_query($conn, $sql2);
