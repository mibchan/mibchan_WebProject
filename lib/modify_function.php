...
function modify_freeboard() {
  ...
  $sql = "update board set title='{$title}', description='{$description}', file='{$file_name}' where num='{$_GET['num']}'";
  switch($file_error) {
    case 0:
...
function modify_infoboard() {
  ...
  $file_upload_error = move_uploaded_file($file_temp, $upload_file);
  $board_name = $_GET['board'];
  $sql = "update board2 set title='{$title}', description='{$description}', file='{$file_name}' where num='{$_GET['num']}'";
  switch($file_error) {
  case 0:
  mysqli_query($conn, $sql);
  echo "<script>alert(\"성공적으로 작성되었습니다.\")</script>";
